...
const UpdateComponent = () => {
    const [student, setStudent] = useState({id: 0, name: "", age: 0, dateOfBirth: "", gender: "true", classCG: 0, subjects: [], address: ''});
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const std = await studentService.findById(id);
            std.classCG = std.classCG.id;
            std.gender = std.gender.toString();
            setStudent(std);
        }
    ...
    const handleSubmit = async (value) => {
        ...
        value.gender = value.gender === "true";
    ...
    return (
        <Formik initialValues={student} onSubmit={handleSubmit} validationSchema={handleValidate} enableReinitialize={true}>
